# Analysis: Lifespan GAM for slope
# Input metric: slope (from full_results.csv)
# Key parameters: Age, ExperimentName (race), Regression (biasp/biasm), Subject
# Output plot: output/plots/GAM_all_groups_slope.png
...
df_grouped <- full_data %>%
  ...
  summarize(slope = mean(slope, na.rm = TRUE), .groups = "drop") %>%
  mutate(
...
p <- ggplot(
  df_grouped,
  aes(x = Age, y = slope, color = Regression, fill = Regression)
) +
  geom_smooth(
    method      = "gam",
    formula     = y ~ s(x, k = 8),
...
# Save output plot
ggsave(
  filename = plot_path("GAM_all_groups_slope.png"),
  plot     = p,
  width    = 6,
  height   = 5,
  dpi      = 300,
...
# Note: method = "ML" for nested model AIC/ANOVA
model_comps <- df_grouped %>%
  group_by(ExperimentName, Regression) %>%
  nest() %>%
  mutate(
    linear_mod = map(data, ~ gam(slope ~ Age,            data = .x, method = "ML")),
    quad_mod   = map(data, ~ gam(slope ~ Age + I(Age^2), data = .x, method = "ML")),
    spline_mod = map(data, ~ gam(slope ~ s(Age, k = 8),  data = .x, method = "ML"))
  ) %>%
  ungroup() %>%
  mutate(
